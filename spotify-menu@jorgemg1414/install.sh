...
if [[ "${1:-}" == "--uninstall" ]]; then
    ...
    rm -rf "$DESTINO"
    # La caché de portadas no está aquí dentro: se borra aparte.
    rm -rf "${XDG_CACHE_HOME:-${HOME}/.cache}/spotify-menu"
...
# La extensión no necesita ningún programa externo: habla con el reproductor por
# el bus de sesión, que ya está ahí. Sí necesita libsoup 3 para bajar las
